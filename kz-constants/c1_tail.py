#!/usr/bin/env python3
"""
c1_tail.py — pin the remaining digits of C_1.
        ...
        if s % 3 == 0:
            ...
            if n <= nmax:
                out[n] = sum(v for (c1, c2, d, r), v in dp.items()
                             if s-c1-c2 == n and c1 == n and c2 == n and r == 2)
                if n % 50 == 0:
                    print("   G(%d): %d digits  [%.0fs]"
...
print("validation against the 17 published terms:",
      "ALL OK" if not bad else "MISMATCH at %s" % bad)
if bad:
    raise SystemExit("aborting")

NS = list(range(args.nmax//2, args.nmax+1))
A = [mpf(G[n]) * mpf(n)**4 / mpf(8)**n for n in NS]
